/**
 * @forgekit/app-shared/pdf — pure-TS PDF writer for Pro-tier /export commands.
 *
 ...
 *
 * Originally validated in SummarizeTube (tick 25/08); lifted here so every
 * bot can export documents without duplicating 200 lines of binary plumbing.
 */
...
export interface PdfDoc {
  ...
  /** Lead paragraph rendered after "<tldrLabel>: ". */
  tldr: string;
  /** Override the lead label for non-summary domains (e.g. "Pergunta"). */
  tldrLabel?: string;
...
export function makeContentStream(doc: PdfDoc): string {
  ...
  for (const line of wrapLine(doc.title?.trim() || "Documento")) show(line);
  show("");
  ...
  const label = doc.tldrLabel?.trim() || "TLDR";
  if (doc.tldr.trim()) {
    for (const line of wrapLine(`${label}: ${doc.tldr.trim()}`)) show(line);
    show("");
  }
...
/**
 * Render the complete PDF bytes for a document.
